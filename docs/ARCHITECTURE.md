...
6) Comment rendering
   ...
   - Uses Python `str.format()` against a context dictionary (see `docs/RULE_AUTHORING.md`).
...
8) Logging (local filesystem side effect)
   - `rackbrain/services/logger.ProcessingLogger` writes JSONL or text.
...
- Logs: JSONL/text logs, default under `logging.log_dir`.
